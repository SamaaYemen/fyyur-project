...
def venues():
  ...
  for location in locations:
    venue_info['city'] = location.city
    venue_info['state'] = location.state
    for venue in all_venues:
      if location.city == venue.city and location.state == venue.state:
        ven_info['id'] = venue.id
        ven_info['name'] = venue.name
        ven_info['num_upcoming-shows'] = 0
        venues.append(ven_info)
        venue_info = {}
    venue_info['venues'] = venues
...
def search_venues():
  ...
  venue_data = {}
  for venue in venues: 
    if search_term.casefold() in venue.name.casefold():
      count += 1
...
def search_artists():
  ...
  artist_data = {}
  for artist in artists: 
    if search_term.casefold() in artist.name.casefold():
      count += 1
...
def create_artist_submission():
  ...
  return render_template('pages/home.html', artist=data)
...
def create_show_submission():
  ...
  try:
     ...
     show = Show(start_time=form.start_time.data, venue_id=v_id, artist_id=artist.id)
